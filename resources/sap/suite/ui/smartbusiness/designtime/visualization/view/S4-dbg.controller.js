...
sap.ca.scfld.md.controller.BaseFullscreenController.extend("sap.suite.ui.smartbusiness.designtime.visualization.view.S4", {
	onInit : function() {
        ...
        var SO_input = this.byId("semanticObjectText");
        SO_input.addEventDelegate({
        	onAfterRendering: function(){
        		that.INIT_SO = this.getValue();
        	}
        }, SO_input);
	},
...
//				});
	...
	getChipUrl: function(tileType) {
		var chipUrls = {
				...
				"DT-TT" : "/sap/bc/ui5_ui5/ui2/ushell/resources/sap/ushell/components/tiles/indicatorDualTrend/DualTrendChip.xml"
		}
		                		 ...
		                		 //usability improvement - Save appParams without explicit click on "+"
		                		   ...
		                		   function publishChipCallBack(data) {
		                			   ...
		                			   if(that.appMode == "addTile") {
		                				   ...
		                				   //sap.suite.ui.smartbusiness.lib.Util.utils.hashChange({hash: "#FioriApplication-configureSBKPITile"});
...
//		this.initialData = jQuery.extend(true, {}, this.tileConfigurationModel.getData(), {});
	...
	handleSemanticObjectChange: function(evt) {
		...
		if(this.tempSemanticObject == "" && this.INIT_SO) {
			evt.getSource().setValue(this.INIT_SO);
		}
	...
	fetchChipData: function(obj) {
		...
		if(this.createMode) {
			...
			if(obj.evalContextPath) {
				...
				evaluationObj = this.metadataRef.getEvaluationById({context:evalContext, texts_chip:true, entity:"EVALUATIONS_MODELER", model:that.oApplicationFacade.getODataModel()});
...
//		});
	...
	selectVizRight : function(oEvent){
	    var rightViz;
	    var dataRef = this.modelRef.getData();

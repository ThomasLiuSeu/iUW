...
sap.ca.scfld.md.controller.BaseDetailController.extend("sap.suite.ui.smartbusiness.designtime.association.view.S4", {
						...
						beginButton: new sap.m.Button({
							...
							press: function(){
								if(that.viewMode=="associationEdit"){
									sap.suite.ui.smartbusiness.lib.Util.utils.replaceHash({action:"manageSBKPIAssociation", route:"detail", context:that.getView().sourceIndicatorContexts.contextPath.replace("INDICATORS","INDICATORS_MODELER")});
								}
								else if(that.viewMode=="associationCreate"){
									sap.suite.ui.smartbusiness.lib.Util.utils.replaceHash({action:"manageSBKPIAssociation", route:"detail", context:that.getView().sourceIndicatorContexts.contextPath});
								}
							}
						}),
	...
	anythingChangedOnBack: function(){
		...
		var updatePropertiesPayload = sap.suite.ui.smartbusiness.lib.Util.utils.dirtyBitCheck({
...
//		}
	...
	handleReverseAssociationDirection : function(){
		...
		if(targetIndicator.length==0){
			...
			sap.suite.ui.smartbusiness.lib.Util.utils.showErrorMessage(that.oApplicationFacade.getResourceBundle().getText("TARGET_KPI_NOT_SELECTED"));
		}
		else{
...
//				}
		...
		if(this.getView().byId("associationPropertyName").getValue() && this.getView().byId("associationPropertyValue").getValue()) {
			...
			if(this.getView().byId("associationPropertyValue").getValue()){
				...
				if(propertyPresent == 0){
					...
					sap.suite.ui.smartbusiness.lib.Util.utils.showErrorMessage(that.oApplicationFacade.getResourceBundle().getText("ERROR_DUPLICATE_PROPERTY_VALUE"));
				}
			}
			else {
				this.getView().byId("associationPropertyValue").setValueState("Error");
				sap.suite.ui.smartbusiness.lib.Util.utils.showErrorMessage(that.oApplicationFacade.getResourceBundle().getText("ENTER_PROPERTY_VALUE"));
			}
		}
		else {
			this.getView().byId("associationPropertyName").setValueState("Error");
			sap.suite.ui.smartbusiness.lib.Util.utils.showErrorMessage(that.oApplicationFacade.getResourceBundle().getText("EMTER_PROPERTY_NAME"));
		}
	},
	...
	activateAssociation: function() {
		...
		/*that.ODataModel.create("/ACTIVE_ASSOCIATIONS", payload, null, function(data) {
                     ...
                     sap.suite.ui.smartbusiness.lib.Util.utils.showErrorMessage(that.oApplicationFacade.getResourceBundle().getText("ASSOCIATION_ACTIVATION_FAILED"));
              });   
		 */
		...
		that.metadataRef.create("ACTIVATE_ASSOCIATION",that.payload,null,
				...
				function(err) {
					sap.suite.ui.smartbusiness.lib.Util.utils.showErrorMessage(that.oApplicationFacade.getResourceBundle().getText("ASSOCIATION_ACTIVATION_FAILED"), err.responseText);
				});
		//that.oApplicationFacade.getODataModel().refresh();
	...
	performBatchOperation : function(batchArray){
		...
		that.oDataModel.submitBatch(function(data,response,errorResponse){
			...
			{      
				sap.suite.ui.smartbusiness.lib.Util.utils.showErrorMessage(that.oApplicationFacade.getResourceBundle().getText("ASSOCIATION_SAVED_FAILED"));
				return;
			}
			var error = false;
	...
	savePropertiesHelper : function(associationPropertiesPayload, updateAssociationPayload, sourceIndicator, targetIndicator, isSaveAndActivate){
		...
		if((that.viewMode === "associationEdit" && !(that.associationData.IS_ACTIVE)) || (that.viewMode === "associationEdit" && that.associationData.IS_ACTIVE && isSaveAndActivate)){
			...
			if(updateAssociationPayload.updates.length!=0 ){
				...
				function(err) {
					sap.suite.ui.smartbusiness.lib.Util.utils.showErrorMessage(that.oApplicationFacade.getResourceBundle().getText("ASSOCIATION_SAVED_FAILED"), err.responseText);
				});
				//that.oApplicationFacade.getODataModel().refresh();
			}
	...
	saveAssociationHelper : function(sourceIndicator,targetIndicator,isSaveAndActivate){
		...
		if((that.viewMode === "associationEdit" && !(that.associationData.IS_ACTIVE)) || (that.viewMode === "associationEdit" && that.associationData.IS_ACTIVE && isSaveAndActivate)){
			...
			updateAssociationPayload = sap.suite.ui.smartbusiness.lib.Util.utils.dirtyBitCheck({
	...
	saveAssociation: function(isSaveAndActivate) {
		...
		if(this.viewMode === "associationEdit") {
			...
			if(targetIndicator==null){
				sap.suite.ui.smartbusiness.lib.Util.utils.showErrorMessage(that.oApplicationFacade.getResourceBundle().getText("SELECT_A_KPI_TO_ASSOCIATE"));

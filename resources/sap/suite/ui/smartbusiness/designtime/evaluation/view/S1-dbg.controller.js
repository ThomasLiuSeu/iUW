...
//		});
		...
		var evaluationFetchCallBack = function(data) {
			...
			for(i=0;i<languageData.length;i++){
				...
				that.oDataModel.read("/LANGUAGE?$filter=SPRAS eq '"+languageData[i].LANGUAGE+"'", null, null, false, function(langData) {
					languageObject.LANGUAGE_KEY = langData.results[0].LAISO;
					languageObject.LANGUAGE = langData.results[0].SPRAS;
	...
	saveSuccessHandling : function(oEvent){
		...
		if(oEvent === "activate" || oEvent === "saveActivate"){
			...
			that.metadataRef.create(url,obj,null,function(data){
				...
				if(oEvent === "activate")
					window.location.hash = "FioriApplication-configureSBKPITile?sap-system=HANA&/detail/EVALUATIONS_MODELER(ID='"+that.getView().getModel().getData().ID+"',IS_ACTIVE=1)";
	...
	saveAndExit : function(oEvent) {
		...
		if(formData.VALUES_SOURCE === "RELATIVE"){
			...
			if(msg!=""){
				//sap.suite.ui.smartbusiness.lib.Util.utils.showErrorMessage(msg);
				//return;
				data.errorMsg.push({
					text : msg,
					icon : "sap-icon://error",
	...
	populateDimensionsAndInputParameters : function(dataSource, entitySet) {
		...
		if(mandatoryFiltersPresent){
			this.byId("mandatoryFiltersLabel").setVisible(true);
			this.byId("mandatoryFiltersDescription").setVisible(true);
			this.byId("mandatoryFilterHeader").setVisible(true);
			this.byId("mandatoryFilterBaseLayout").setVisible(true);
		}
		else{
			this.byId("mandatoryFiltersLabel").setVisible(false);
			this.byId("mandatoryFiltersDescription").setVisible(false);
			this.byId("mandatoryFilterHeader").setVisible(false);
			this.byId("mandatoryFilterBaseLayout").setVisible(false);
		}
		...
		{                              
			...
			if (inputParameters.length > 0){
				...
				this.byId("noparameterlabel").setVisible(false);
				this.byId("noparameterrequired").setVisible(false);
			}
			else{
				this.byId("inputParameterLayoutHeaders").setVisible(false);
				this.byId("noparameterlabel").setVisible(true);
				this.byId("noparameterrequired").setVisible(true);
			}
	...
	setInputParameterAndFilterLayout : function() {
		...
		this.byId("mandatoryFilterBaseLayout").bindAggregation("items", "/selectedDimensions", function(sId, oContext) {
			var mandatoryFilterOperator = new sap.m.Select({
				width : "100%",
				customData : [{
					key : "valueType",
					value : "value_1"
			...
			var mandatoryFilterValue = new sap.m.Input({
				value : "{value_1}",
				customData : [{
					key : "valueType",
					value : "value_1"
			...
			var mandatoryFilterGrid = new sap.ui.layout.Grid({
				defaultSpan : "L9 M9",
				content : [new sap.m.Input({
					value : "{name}",
					width : "100%",
				...
				}), mandatoryFilterOperator, mandatoryFilterValue]
			});
			return mandatoryFilterGrid;
		});
	...
	addAdditionalLanguageDialog : function(){
			            	        	  ...
			            	        	  content : [
			            	        	             ...
			            	        	             new sap.m.Button({
			            	        	            	 ...
			            	        	            	 press : function(){
			            	        	            		 ...
			            	        	            		 if(that.addedLanguagesList.getItems().length>0)
			            	        	            		 {
		...
		if(that.getView().getModel().getData().NO_OF_ADDITIONAL_LANGUAGES>0){
			additionalLanguageDialog.getBeginButton().setEnabled(true);
		}else{
			additionalLanguageDialog.getBeginButton().setEnabled(false);
		}
	...
	multiMeasureTokenChange : function(oEvent){
		...
		for(var i=0,l=measures.length; i<l; i++) {
			if(measures[i].getText() == that.getView().getModel().getData().COLUMN_NAME) {
				that.byId("additionalMeasures").setValueState("Error");
				//sap.suite.ui.smartbusiness.lib.Util.utils.showErrorMessage(this.getView().getViewData().controller.oApplicationFacade.getResourceBundle().getText("ADDI_MEASURE_HAS_MAIN_MEASURE"));
				return;
			}
			else{
	...
	multiMeasureChange : function(value){
		...
		for(var i=0,l=measures.length; i<l; i++) {
			if(measures[i].getText() == that.getView().getModel().getData().COLUMN_NAME) {
				that.byId("additionalMeasures").setValueState("Error");
				//sap.suite.ui.smartbusiness.lib.Util.utils.showErrorMessage(that.oResourceBundle.getText("ADDI_MEASURE_HAS_MAIN_MEASURE"));
				return;
			}
			else{
				that.byId("additionalMeasures").setValueState("None");

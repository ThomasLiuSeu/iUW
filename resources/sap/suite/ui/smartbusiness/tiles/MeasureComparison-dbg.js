	...
	for(i=0, l=measures.length; i < l; i++) {
		...
		if(this._bUseFormattedVDMValue && VDMFormattedProperty && (VDMFormattedProperty != measureName)) {
	        ...
	        if(this.evaluationApi.getScaling() == -2) 
	            formattedValue += " %"
	        */
	        formattedValue = sap.suite.ui.smartbusiness.lib.Util.utils.getLocaleFormattedValue(calculatedValueForScaling, this.evaluationApi.getScaling(),this.evaluationApi.getDecimalPrecision());
		    if(this.evaluationApi.getScaling() == -2) {

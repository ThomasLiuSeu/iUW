...
sap.uiext.inbox.SubstitutionRulesManager.prototype._fetchSubstitutionRuleCollectionData = function() {
	...
	var that = this;
	that.oDataManager.readData("/SubstitutionRuleCollection", {
		success : function(oData,oResponse) {
			that._getSubstitutionRuleCollectionModel().setData({SubstitutionRuleCollection:oData.results});
		},
...
sap.uiext.inbox.SubstitutionRulesManager.prototype.open = function() {
	...
	//setting flag bSubstitutionProfileAvailable
	var oProfileCollectionConstant = this.substitutionConstants.SubstitutionProfileCollection;
	this.bSubstitutionProfileAvailable = this.oTcmMetadata._isEntitySet(oProfileCollectionConstant.name, oProfileCollectionConstant.entityType);
...
sap.uiext.inbox.SubstitutionRulesManager.prototype.populateOverlayContainer = function() {
	...
	if (oSRMOverlayContainer === undefined) {
		...
		oSRMOverlayContainer.attachClose(function(oEvent) {
			...
			//resetting the variables to refresh the data if user opens substitution screen after closing it once.
			...
			// bSystemInfoModelDataSet is a boolean variable which indicates whether SystemInfoCollection model's data has been set
			if (that.bSystemInfoModelDataSet) {
				that.bSystemInfoModelDataSet = false;
			}
			
			if (that._fetchSystemInfoCollectionStarted) {
				that._fetchSystemInfoCollectionStarted = undefined;
			}
	...
	if (createSubsRulePopup === undefined) {
		...
		popupValueHelp.addDelegate({
	        onAfterRendering:function() {
	        	jQuery("#" + popupValueHelp.getId() + "-input").prop("maxLength", 0);
	        	jQuery("#" + popupValueHelp.getId() + "-input").prop("readonly", "readonly");
	        }
	    });
		...
		if (oSubtRuleManager.bSubstitutionProfileAvailable) {
			...
			var oSubstProfileValueHelp = new sap.ui.commons.ValueHelpField(dynamicId + 'substProfileValueHelp', {
				...
				onAfterRendering:function() {
					jQuery("#" + oSubstProfileValueHelp.getId() + "-input").prop("maxLength", 0);
					jQuery("#" + oSubstProfileValueHelp.getId() + "-input").prop("readonly", "readonly");
				}
	...
	if (oSubstRuleRowRepater === undefined) {
		...
		if (that.bSubstitutionProfileAvailable) {
			...
			var rowRepSystemInfoTxt = new sap.ui.commons.TextView(dynamicId + 'rowRepSystemInfoTxt').bindText({
				...
				parts: [ {path: "SAP__Origin", type: new sap.ui.model.type.String()},
			             {path: "SystemInfoCollectionModel>SystemAlias", type: new sap.ui.model.type.String()} ],
			             
			    formatter: function(sSapOrigin, sSystemAlias){
			    	if (sSapOrigin) {
			    		// seting data in SystemInfoCollectionModel if it has not been set already
			    		// TODO varify it. Logic needs to be refactored. Delete that un necessary flag _fetchSystemInfoCollectionStarted.
			    		if (!that.bSystemInfoModelDataSet && !that._fetchSystemInfoCollectionStarted) {
			    			that._fetchSystemInfoCollectionModelData();
			    			that._fetchSystemInfoCollectionStarted = true;
			    		}
			    		
			    		if (sSystemAlias != null && sSystemAlias !== undefined && sSystemAlias !== "") {
			    			return that._oBundle.getText("SUBSTITUTION_RULE_SYSTEM_ALIAS") + " : " + sSystemAlias;
			    		} else {
			    			this.bindElement("SystemInfoCollectionModel>/SystemInfoCollection/" + sSapOrigin + "/");
			    		}
			        }
			    }
		...
		// following model contains the information of configured backend systems and their system alias names
		oSubstRuleRowRepater.setModel(this._getSystemInfoCollectionModel(), "SystemInfoCollectionModel");
...
sap.uiext.inbox.SubstitutionRulesManager.prototype._handleSearchUsersClickforCreateSubstitution = function(oEvent){
	...
	var fnTriggerSearch = jQuery.proxy(function(){
		
		// sending one SAP__Origin in case of multi providers
		if (this.bSystemInfoAvailable) {
			var oSystemInfoCollectionData = this._getSystemInfoCollectionModel().oData.SystemInfoCollection;
			for (var elem in oSystemInfoCollectionData) {
				sSAPOrigin = oSystemInfoCollectionData[elem].SapOrigin;
				break;
			};
		}
	...
	if (this.bSystemInfoAvailable && !this.bSystemInfoModelDataSet) {
		this._fetchSystemInfoCollectionModelData(fnTriggerSearch);
	} else {
		fnTriggerSearch();
	}		
...
sap.uiext.inbox.SubstitutionRulesManager.prototype._readSystemInfoData = function(fnSuccess){
	
	var that = this;
	this.oDataManager.readData("/SystemInfoCollection", {
		success : function(oData,oResponse) {
			if (fnSuccess) {
				fnSuccess(oData);
			}
		...
		error : function(error) {
			that.showMessage("error", that._oBundle.getText("INBOX_MSG_FETCH_SYSTEM_INFO_COLLECTION_FAILED"));
		}
	});
...
sap.uiext.inbox.SubstitutionRulesManager.prototype._getProvidersForCreateSubstitution = function(oEntry, fnCreateSubRuleRequest){
	...
	if (sProfileValueHelp != null && sProfileValueHelp !== "") {
		...
		var fnSendRuleRequest = jQuery.proxy(function() {
			var oSystemInfoCollectionData = this._getSystemInfoCollectionModel().oData.SystemInfoCollection;
			for (var elem in oSystemInfoCollectionData) {
				aSelectedProviders.push(oSystemInfoCollectionData[elem].SapOrigin);
			};
			
			fnCreateSubRuleRequest(aSelectedProviders);
		}, this);
		
		if (!this.bSystemInfoModelDataSet) {
			this._fetchSystemInfoCollectionModelData(fnSendRuleRequest);
		} else {
			fnSendRuleRequest();
		}
...
sap.uiext.inbox.SubstitutionRulesManager.prototype._fetchSystemInfoCollectionModelData = function(fnAfterFetch) {
	
	var fnSuccessForReadSystemData = jQuery.proxy(function(oSystemsData) {
		
		if (oSystemsData) {
			var oSystemInfoData = [];
			jQuery.each(oSystemsData.results, function(j, item) {
				oSystemInfoData[item.SAP__Origin] = { SapOrigin : item.SAP__Origin, SystemAlias : item.SystemAlias };
			});
			this._getSystemInfoCollectionModel().setData({"SystemInfoCollection": oSystemInfoData});
			this.bSystemInfoModelDataSet = true;
		}
		...
		if (this._fetchSystemInfoCollectionStarted) this._fetchSystemInfoCollectionStarted = undefined;
		
	}, this);
	
	this._readSystemInfoData(fnSuccessForReadSystemData);

};

sap.uiext.inbox.SubstitutionRulesManager.prototype._getSystemInfoCollectionModel = function(){
	if(!this.oSystemInfoCollectionModel) {
		this.oSystemInfoCollectionModel = new sap.ui.model.json.JSONModel();
		this.oSystemInfoCollectionModel.setData({"SystemInfoCollection": []});
	}
	return this.oSystemInfoCollectionModel;
};

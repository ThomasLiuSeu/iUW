...
(function () {
    ...
    function initBootstrapPlugins(oConfig) {
        var oBootstrapPluginsCfg = oConfig.bootstrapPlugins,
            sModuleName;

        if (oBootstrapPluginsCfg) {
            Object.keys(oBootstrapPluginsCfg).forEach(function (sPluginCfgKey) {
                sModuleName = oBootstrapPluginsCfg[sPluginCfgKey].module;

                // TODO: consider "real" async loading of module
                try {
                    jQuery.sap.require(sModuleName);
                } catch (e) {
                    jQuery.sap.log.error(e, "Error when loading bootstrap plugin: " + sPluginCfgKey);
                }
            });
        }
    }

	...
	sap.ui.controller("tiles.indicatorHarveyBall.HarveyBallTile", {
		...
		attachHandlers:function(){
			...
			this.getVariable("tile").attachPress(function(){
				that.abortAllOpenODataRequests();
				var evalData=that.getVariable("evalData");
		...
		loadPreviewData:function(){
			...
			this.getView().getModel().setData({
				...
				header:oTitle.title||this.getVariable("indicatorUtils").getChipTitle(that.getVariable("evalData")),
				subheader :oTitle.subTitle||this.getVariable("indicatorUtils").getChipSubTitle(that.getVariable("evalData"))
			});
		},
        	...
        	if(this.getVariable("tileProperties").isFractionMeasure){
        		...
        		if(goalType == "MA"){
        			...
        			if(this.hasSomeValue(WL,CL)){
						color = kpiValue <CL.VALUE ?"Error":(kpiValue <= WL.VALUE)?"Critical":"Good"
					}
        			...
        			if(this.hasSomeValue(WH,CH)){
						color =  kpiValue <WH.VALUE ?"Good":(kpiValue <= CH.VALUE)?"Critical":"Error";
					}
        		} else {
            		WL = oThreshold.WL;
            		CL = oThreshold.CL;
		...
		onKpiValueFetched:function(kpiValue,kpiValueUnit,fractionValue,fractionValueUnit){
			...
			scaledKpiValue = indicatorUtil.getLocaleFormattedValue(Number(scaledKpiValue),scalingFactor);
			scaledFractionValue = indicatorUtil.getLocaleFormattedValue(Number(scaledFractionValue),scalingFactor);
